...
protected:
    ...
    void XmlEncodedKeyIdentifier(AuthorityKeyIdentifierExtension ext, std::string tab = "") {
      std::string expected = tab + "<authorityKeyIdentifier>\n" +
        tab + "\t<extnID>authorityKeyIdentifier</extnID>\n" +
        tab + "\t<critical>no</critical>\n" +
        tab + "\t<extnValue>\n" +
        tab + "<keyIdentifier>" + ext.getKeyIdentifier().toHex() + "</keyIdentifier>\n" +
        tab + "\t</extnValue>\n" + 
        tab + "</authorityKeyIdentifier>\n";
    }
    ...
    void ext2Xml(AuthorityKeyIdentifierExtension ext) {
      auto data = ByteArray(info);
      ext.setKeyIdentifier(data);
      std::cout << ext.getKeyIdentifier().toHex() << std::endl;
      ASSERT_EQ(ext.extValue2Xml(), "<keyIdentifier>" + Base64::encode(data) + "</keyIdentifier>\n");
    }
...
TEST_F(AuthorityKeyIdentifierExtensionTest, XmlTestValue) {
  XmlEncodedKeyIdentifier(ExtensionConstructor());
}

TEST_F(AuthorityKeyIdentifierExtensionTest, XmlTestValueTabed) {
  XmlEncodedKeyIdentifier(ExtensionConstructor(), "tab");
}
...
TEST_F(AuthorityKeyIdentifierExtensionTest, Ext2XmlTest) {
